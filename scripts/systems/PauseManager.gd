...
func setup(d_mgr: DialogueManager, q_mgr: QuizManager, dom_ui: DomainSelectionUI, pause_ui: PauseMenuUI) -> void:
...
func handle_escape_pressed() -> void:
	...
	# Priority 3: ELSE IF domain selection is active -> cancel selection
	if domain_ui and domain_ui.is_open():
		domain_ui.cancel_selection()
		return
		
	# Priority 4: ELSE IF quiz is active -> cancel quiz attempt
	if quiz_manager and quiz_manager.is_active():
		quiz_manager.cancel_quiz()
		return
		
	# Priority 5: ELSE IF pause menu is open -> close pause menu, resume gameplay
	if pause_menu_ui and pause_menu_ui.is_open():
		pause_menu_ui.close_pause_menu()
		return
		
	# Priority 6: ELSE -> open pause menu
	if pause_menu_ui:
