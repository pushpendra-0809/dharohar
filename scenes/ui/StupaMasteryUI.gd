...
func _setup_button_events() -> void:
	...
	for idx in range(opt_btns.size()):
		var opt = opt_btns[idx]
		if opt and not opt.pressed.is_connected(_on_option_selected.bind(idx)):
			opt.pressed.connect(_on_option_selected.bind(idx))
			_setup_hover(opt)
...
func _on_btn_hover(btn: Control, zoom: bool) -> void:
	...
	var sc = Vector2(1.03, 1.03) if zoom else Vector2(1.0, 1.0)
...
func open_ui(selected_domain: String = "") -> void:
	visible = true
	if color_rect:
	...
	if selected_domain != "":
		_on_domain_selected(selected_domain)
	else:
		_show_domain_view()
...
func _update_progress_header() -> void:
	...
	var completed_count: int = GameState.get_stupa_completed_count()
	var hard_count: int = GameState.get_stupa_hard_completed_count()
	
	if progress_label:
		progress_label.text = "Stupa Trials Completed: " + str(completed_count) + " / 12  (" + str(hard_count) + "/4 Domain Scrolls)"
		
	if scroll_status_label:
		if GameState.stupa_scroll_earned:
			scroll_status_label.text = "📜 STUPA SCROLL EARNED!"
			scroll_status_label.add_theme_color_override("font_color", Color(1.0, 0.88, 0.2))
		else:
			scroll_status_label.text = "🔒 Stupa Scroll: Complete all 4 Hard Challenges"
...
func _update_domain_badges() -> void:
	...
	for i in range(doms.size()):
		...
		var lbl = labels[i]
		if lbl:
			var e = "✓" if GameState.is_stupa_challenge_completed(d, "easy") else "—"
			var m = "✓" if GameState.is_stupa_challenge_completed(d, "medium") else "—"
			var h = "★" if GameState.is_stupa_challenge_completed(d, "hard") else "—"
...
func _update_difficulty_buttons() -> void:
	...
	var e_done = GameState.is_stupa_challenge_completed(current_domain, "easy")
	var m_done = GameState.is_stupa_challenge_completed(current_domain, "medium")
	var h_done = GameState.is_stupa_challenge_completed(current_domain, "hard")
	
	var m_unlocked = GameState.is_stupa_difficulty_unlocked(current_domain, "medium")
	var h_unlocked = GameState.is_stupa_difficulty_unlocked(current_domain, "hard")
...
func _start_challenge(domain: String, diff: String) -> void:
	current_challenge_data = StupaChallengeData.get_challenge(domain, diff)
	...
	var dom_info = StupaChallengeData.CHALLENGES.get(domain, {})
...
func _on_option_selected(idx: int) -> void:
	...
	if is_correct:
		var exp_reward: int = GameState.STUPA_EXP_REWARDS.get(current_difficulty, 25)
		if feedback_title:
		...
		if GameState:
			GameState.complete_stupa_challenge(current_domain, current_difficulty)
		...
		if explanation_label:
			explanation_label.text = "Reflect on the ancient observation and principles. Try again!"
