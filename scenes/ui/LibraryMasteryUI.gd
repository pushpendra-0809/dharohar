...
func _ready() -> void:
	add_to_group("library_mastery_ui")
	visible = false
	if color_rect:
...
func _update_progress_header() -> void:
	...
	var completed_count: int = GameState.get_library_completed_count()
	var hard_count: int = GameState.get_library_hard_completed_count()
	
	if progress_label:
		progress_label.text = "Library Trials Completed: " + str(completed_count) + " / 12  (" + str(hard_count) + "/4 Domain Scrolls)"
		
	if scroll_status_label:
		if GameState.library_scroll_earned:
			scroll_status_label.text = "📜 LIBRARY SCROLL EARNED!"
			scroll_status_label.add_theme_color_override("font_color", Color(1.0, 0.88, 0.2))
		else:
			scroll_status_label.text = "🔒 Library Scroll: Complete all 4 Hard Challenges"
...
func _update_domain_badges() -> void:
	...
	for i in range(doms.size()):
		...
		var lbl = labels[i]
		if lbl:
			var e = "✓" if GameState.is_library_challenge_completed(d, "easy") else "—"
			var m = "✓" if GameState.is_library_challenge_completed(d, "medium") else "—"
			var h = "★" if GameState.is_library_challenge_completed(d, "hard") else "—"
...
func _update_difficulty_buttons() -> void:
	...
	var e_done = GameState.is_library_challenge_completed(current_domain, "easy")
	var m_done = GameState.is_library_challenge_completed(current_domain, "medium")
	var h_done = GameState.is_library_challenge_completed(current_domain, "hard")
	
	var m_unlocked = GameState.is_library_difficulty_unlocked(current_domain, "medium")
	var h_unlocked = GameState.is_library_difficulty_unlocked(current_domain, "hard")
...
func _start_challenge(domain: String, diff: String) -> void:
	current_challenge_data = LibraryChallengeData.get_challenge(domain, diff)
	...
	var dom_info = LibraryChallengeData.CHALLENGES.get(domain, {})
...
func _on_option_selected(idx: int) -> void:
	...
	if is_correct:
		var exp_reward: int = GameState.LIBRARY_EXP_REWARDS.get(current_difficulty, 25)
		if feedback_title:
		...
		if GameState:
			GameState.complete_library_challenge(current_domain, current_difficulty)
		...
		if explanation_label:
			explanation_label.text = "Study the ancient records and clues carefully. Try again!"
